...
class testmatchPriority(unittest.TestCase):
	...
	def test_matchPriority(self):
		...
		sleep(5)
		driver.find_element_by_xpath("//li[contains(@url,'tCCdMatchPriority/prepareExecute')]").click()
		driver.implicitly_wait(10)
		driver.switch_to.frame(
		driver.find_element_by_xpath("//iframe[contains(@src,'prepareExecute/toQueryPage')]"))  # 跳转到右侧iframe
		sleep(2)
		driver.find_element_by_xpath("//table[contains(@id,'btn_tname')]/tbody/tr/th/div/input").click()
		sleep(3)
		driver.find_element_by_xpath("//a[text()='修改']").click()
		sleep(2)
		driver.find_element_by_xpath("//select[@id='dtopriority' and @name='priority']").click()
		Select(driver.find_element_by_xpath("//select[@id='dtopriority' and @name='priority']")).select_by_visible_text("高")
		sleep(3)
